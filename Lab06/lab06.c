...
/*			if (temneighbor>0) {map[i][j]=minneighbor;} */
			...
			if ((map[i][j]>map[i-1][j-1])&&(map[i-1][j-1]!=1)&&(map[i-1][j-1]!=0)) {map[i][j]=map[i-1][j-1];}
			if ((map[i][j]>map[i-1][j])&&(map[i-1][j]!=1)&&(map[i-1][j]!=0))       {map[i][j]=map[i-1][j];}
			if ((map[i][j]>map[i-1][j+1])&&(map[i-1][j+1]!=1)&&(map[i-1][j+1]!=0)) {map[i][j]=map[i-1][j+1];}
			if ((map[i][j]>map[i][j-1])&&(map[i][j-1]!=1)&&(map[i][j-1]!=0))       {map[i][j]=map[i][j-1];}
			if ((map[i][j]>map[i][j+1])&&(map[i][j+1]!=1)&&(map[i][j+1]!=0))       {map[i][j]=map[i][j+1];}
			if ((map[i][j]>map[i+1][j-1])&&(map[i+1][j-1]!=1)&&(map[i+1][j-1]!=0)) {map[i][j]=map[i+1][j-1];}
			if ((map[i][j]>map[i+1][j])&&(map[i+1][j]!=1)&&(map[i+1][j]!=0))       {map[i][j]=map[i+1][j];}
			if ((map[i][j]>map[i+1][j+1])&&(map[i+1][j+1]!=1)&&(map[i+1][j+1]!=0)) {map[i][j]=map[i+1][j+1];}
